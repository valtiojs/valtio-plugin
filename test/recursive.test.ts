...
describe('Nested Instance System', () => {
  ...
  describe('Plugin inheritance', () => {
    it('should inherit plugins from parent to child', () => {
      ...
      expect(parentPlugin.beforeChange).toHaveBeenCalledWith(
        ['count'],
        1,
        0,
        expect.any(Object)
      );
      expect(parentPlugin.afterChange).toHaveBeenCalledWith(
        ['count'],
        1,
        expect.any(Object)
      );
  ...
  describe('Subscribe and snapshot with nested instances', () => {
    ...
    it('should handle snapshot with inherited plugins', () => {
      ...
      expect(parentPlugin.onSnapshot).toHaveBeenCalledWith(
        expect.objectContaining({ count: 42 })
      );
      expect(childPlugin.onSnapshot).toHaveBeenCalledWith(
        expect.objectContaining({ count: 42 })
      );
...
// Fixed recursive tests - replace onGet with transformGet where return values are expected
